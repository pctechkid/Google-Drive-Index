...
function nav(path) {
  ...
  <a class="navbar-brand" href="/${cur}:/"><i class="fa fa-cloud" aria-hidden="true"></i>&nbsp;&nbsp;${UI.logo_image ? '<img border="0" alt="'+UI.company_name+'" src="'+UI.logo_link_name+'" height="30px">' : UI.logo_link_name}</a>
	...
	$.post(`/${cur}:id2path`, {
		...
		if (data) {
			...
			dialog = mdui.dialog({
				...
				content: `<a class="btn btn-info" href="${hrefurl}">Open</a> <a class="btn btn-success" href="${hrefurl}" target="_blank">Open in New Tab</a> <button class="btn btn-danger" mdui-dialog-cancel>Cancel</button><script>dialog.addEventListener('cancel.mdui.dialog', function () {
  console.log('cancel');
});</script>`,
				history: false,
				modal: true,
...
function file_video(path) {
  ...
  <div class="btn-group-vertical" role="group">
		  <button class="btn btn-success mx-auto" href="${dl}"><i class="fa fa-download"></i> &nbsp;   Download</a>
		  <button class="btn btn-primary mx-auto" href="intent:${url}#Intent;package=com.mxtech.videoplayer.pro;B.secure_uri=true;end"><i class="fa fa-play-circle"></i> &nbsp;   MX Player Pro</a>
		  <button class="btn btn-danger mx-auto" href="vlc://${vlc}"><i class="fa fa-play-circle"></i> &nbsp;   VLC Media Player</a>
  </div>	
